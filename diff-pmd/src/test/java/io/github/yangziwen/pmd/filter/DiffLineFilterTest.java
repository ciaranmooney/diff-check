...
public class DiffLineFilterTest {
    ...
    public void insertEditWithViolation() {
        ...
        DiffEntryWrapper wrapper = buildWrapper(Collections.singletonList(insert));
        DiffLineFilter diffLineFilter = new DiffLineFilter(Collections.singletonList(wrapper));
        RuleViolation violation = new DummyRuleViolation(7, 7, FILE_PATH);
        boolean result = diffLineFilter.accept(violation);
    ...
    @Test
    public void insertEditWithSecondMatchingViolation() {
        Edit nonViolatingEdit = new Edit(1, 1, 1, 4);
        Edit insert = new Edit(6, 6, 6, 7);
        DiffEntryWrapper wrapper = buildWrapper(Arrays.asList(nonViolatingEdit, insert));
        DiffLineFilter diffLineFilter = new DiffLineFilter(Collections.singletonList(wrapper));
        RuleViolation violation = new DummyRuleViolation(7, 7, FILE_PATH);
        boolean result = diffLineFilter.accept(violation);
        Assert.assertTrue(result);
    ...
    public void replaceEditWithOriginalViolation() {
        ...
        List<DiffEntryWrapper> diffEntryList = Collections.singletonList(buildWrapper(Collections.singletonList(replace)));
        DiffLineFilter diffLineFilter = new DiffLineFilter(diffEntryList);
    ...
    private static DiffEntryWrapper buildWrapper(List<Edit> edits) {
        return DiffEntryWrapper.builder()
                .diffEntry(new DummyDiffEntry(DiffLineFilterTest.FILE_PATH))
                .gitDir(new File(ROOT_PATH))
                .editList(edits).build();
    }
